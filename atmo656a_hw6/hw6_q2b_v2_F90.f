...
C
     ...
     2         rin,  rfri, rfii, wnum
      ...
      DIMENSION  ALAM(4),  RFRS(4),  RFIS(4),  RSH(10000)

      DATA  ALAM /  0.55, 1.00, 1.60, 2.20 /
      DATA  RFRS /  1.3873, 1.302, 1.290, 1.263 /
      DATA  RFIS /  0.0066,  0.193E-05, 0.213E-03, 0.800E-03 /
      RSH(1) = 0.001
      DO I=2,10000
        RSH(I) = RSH(I-1) + 0.001
      END DO
...
c     RFII  =  0.0
c     RIN   =  0.0 
      open(1, file = 'hw6_q2b_v2_output.txt', status = 'new')
      DO 40  I=1,1
      WNUM   =  TWOPI / ALAM(I)
      RIN    =  RSH(I)
      RFRO   =  RFRS(I)
...
c      open(1, file = 'hw6_q2a_v2_output.txt', status = 'new')
...
c	 WRITE( 6,*)  ROUT
     ...
     4                 LL,      RIN,     RFRI,    RFII,    WNUM   )
         ...
         write( 1,* ) ROUT, QEXT, QABS, QBS, SCAT, asym   
c      close(1)
...
c      open(1, file = 'hw6_q2a_v2_output.txt', status = 'old') 
